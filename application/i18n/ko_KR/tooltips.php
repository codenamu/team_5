...
$lang = array(
	'actions' => array(
		'add_to_category' => '이 기능은 하나의 데이터에 추가 카테고리를 설정할 수 있게 합니다. 예를 들면 카테고리 1을 선택하고 카테고리 2를 추가로 선택하면, 해당 데이터는 2개의 카테고리에 속하게 됩니다.',
		'approve' => '데이터를 승인 또는 하지 않음. 만약 승인하면 데이터는 공개됩니다.',
		...
		'category' => '만약에 특정 카테고리가 사용이 될 때 트리거를 활성화하고자 한다면 여기서 설정을 할 수 있습니다. 카테고리 중의 하나가 사용될 경우에만 활성화 될 것입니다. 예를 들면 카테고리 1과 카테고리 2를 선택하고 데이터가 카테고리 2와 카테고리 3에 등록되면, 그것은 테스트를 통과 할 것입니다.',
		...
		'location' => '다양한 지역 선택이 가능합니다. 만약, 특정 지역을 선택하고 해당 지역에서만 데이터를 받을 수 있습니다. 예를 들어 누군가로부터 브라질에 관련한 데이터를 받길 원하면, "특정지역"을 선택하고, 브라질 주위에 박스를 그려 이를 설정할 수 있습니다. 이 박스의 크기는 자유롭게 설정할 수 있으며, 1개 이상의 박스를 그릴 수 있습니다.',
		'on_specific_count' => '이것은 전체 유저 베이스 또는 개별 유저 중 하나에서 특정번째에서 작동되게 할 수 있습니다. 사용하지 않으려면 빈칸으로 두면 됩니다.',
		'report_title' => '데이터의 기본 제목입니다.',
		'response' => '만약, 모든 통과자들이 통과하면, 트리거는 응답을 시작합니다. 이것은 데이터를 승인하고 이메일로 사용자에게 알리는 것을 말합니다. 여기서 세부적인 응답내용을 추가하고 설정할 수 있습니다.',
		'send_to' => '만약, 트리거 사용자를 선택하면, 특정 액션을 실행한 사용자에게 이메일을 보냅니다. 만약,  라디오 버튼을 선택하면 이메일 주소를 입력할 수 있습니다. 이것은 특정 지역상의 데이터를 보거나 체크인등 하는 이들에게 트리거 할 수 있게 해줍니다.',
		'specific_days' => '복수일을 이곳에서 지정할 수 있습니다. 날짜는 개발 시간대에 의해 정해집니다. 모든 일 선택을 기본으로 하기 위해서는 어떤 날짜도 선택하지 마세요.',
		'trigger' => '트리거는 액션트리거 설정의 주요 구성요소입니다. 누군가 데이터를 공유하고, 체크인등을 실행할 때 특정 활동이 일어날 수 있도록 결정할 수 있습니다. 그리고 하나를 선택한 후 이러한 액션에 대해 필터링 할 수 있습니다.',
		'user' => '트리거 사용자는 누구나 또는 특정사용자가 될 수 있습니다. 만약, 특정사용자에게 트리거가 활성화되게 하려면, 여기서 선택해야 합니다. 아니면, 모든사용자에게 트리거를 설정되게 됩니다.',
		'verify' => '확인되었거나 확인 되지 않은 데이터를 표시합니다.',
	) ,
	...
	'profile_notify' => '"네"를 선택하면 새로운 데이터나 댓글이 있을 때, 이메일로 알림을 보내줍니다.',
	'profile_password' => '계정에 대해 허가되지 않은 변경 시에는 현재 암호를 입력해야 합니다.',
	'profile_public' => '이 옵션을 체크하면 당신의 프로파일은 공개됩니다. 또한 당신이 올린 데이터, 체크인, 배지 등을 하나의 페이지에 보여줄 수 있는 쉬운 방법입니다.',
	...
	'settings_allow_clustering' => '유사한 데이터를 하나의 점으로 묶어 보여줍니다.',
	'settings_allow_comments' => '사용자의 댓글을 허용합니다.',
	'settings_allow_feed' => '메인 사이트에서 RSS 뉴스 피드가 노출됩니다.',
	'settings_allow_reports' => '웹양식을 통해 정보를 공유할 수 있습니다.',
	...
	'settings_checkins' => '이 셋팅은 현재 배치에서 체크인을 가능하게 합니다. 이것은 홈페이지에서 요구하는 특정한 방식을 따르지 않아도 되는 매우 간단한 데이터 방식입니다. 만약, 이 기능을 사용하려면 타임존-UTC기준- 과 테마가 지원하는 지 확인해야 합니다. 사용중일 때, 체크인 전용 테마는 Addons 메뉴에서 확인 가능합니다.',
	...
	'settings_display_items_per_page' => '메인사이트에서 페이지당 보여 줄 데이터 숫자',
	'settings_display_items_per_page_admin' => '관리자페이지에서 페이지당 보여 줄 데이터 숫자',
	...
	'settings_map_timeline' => '데이터가 수신된 날짜와 시간에 따른 타임라인입니다.',
	...
	'settings_server_password' => '데이터를 수신하는 이메일의 암호입니다.',
	...
	'settings_server_username' => '데이터를 받는 이메일주소입니다.',
	...
	'settings_site_email' => '이메일 데이터와 문의하기를 통해 들어오는 메시지를 받는 이메일입니다.',
	...
	'settings_site_submit_report_message' => '데이터 공유 페이지에 나타나는 메시지입니다. 면책조항이나 사용방법을 사용자에게 알려줄 때 유용합니다.',
